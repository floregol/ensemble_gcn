...
def get_trained_gcn(seed, dataset, y_train, y_val, y_test, train_mask, val_mask, test_mask, ensemble=False, theta=None, Gamma=None):
    ...
    # Create model
    model = model_func(
        placeholders, input_dim=features[2][1], ensemble=ensemble, theta=theta, Gamma=Gamma,  logging=True)
    ...
    def softmax(features):
        ...
        all_softamx_output, _, _ = sess.run(
            [model.softmax_ouput, model.loss, model.accuracy], feed_dict=feed_dict_val)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        if VERBOSE:
            ...
            print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(outs[1]), "train_acc=",
                  "{:.5f}".format(outs[2]), "val_loss=", "{:.5f}".format(
                      cost), "val_acc=", "{:.5f}".format(acc),
